...
get_beta <- function(d, ys, f) {
  ...
  if (length(unique(d$sess_exper)) > 1) {
    f <- paste(f, 'sess_exper', sep = ' + ')
    # to prevent empty factor errors in model
    d$sess_exper = as.character(d$sess_exper)
  }
  # scale x's
...
for (s in subj){
  ...
  if (dim(node_data)[1] > 0){
    ...
    net_data <- group_by(node_data, sess_exper, time, spike_num, bin_spike, fc_measure, band, band_measure) %>%
      dplyr::summarise(str = mean(str, na.rm=TRUE), str_soz = mean(str_soz, na.rm=TRUE), str_not_soz = mean(str_not_soz, na.rm=TRUE),
                       str_spike = mean(str_spike, na.rm=TRUE), str_not_spike = mean(str_not_spike, na.rm=TRUE), power = mean(power))
    # add constants
