...
for (s in subj){
  ...
  if (file.exists(curr_csv) ){
    ...
    if (dim(node_data)[1] > 0){
      ...
      # get network level strength separated by groups
      net_data <- group_by(node_data, tid, spike_num, spike_spr, bin_spike) %>%
        dplyr::summarise(str = mean(str, na.rm=TRUE))
      alt_net <- group_by(alt_data, tid, spike_num, spike_spr, bin_spike)
      num_corr = cor.test(net_data$spike_num, alt_net$spike_num, method='spearman')
      spr_corr = cor.test(net_data$spike_spr, alt_net$spike_spr, method='spearman')
      #  concetenate
      params <- suppressWarnings(bind_rows(data.frame( subj =curr_subj[[1]][3], default_num=net_data$spike_num, 
                                                       alt_num=alt_net$spike_num,corr_num=num_corr$estimate, 
                                                       default_spr=net_data$spike_spr, alt_spr=alt_net$spike_spr,
                                                       corr_spr=spr_corr$estimate, params))) # silence warnings about converting factors to strings
    }
  }
}
