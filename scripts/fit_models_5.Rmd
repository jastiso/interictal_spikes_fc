...
# final df
col_names <- c("fc_measure", "band", 'band_measure', "str_beta_bin", "str_beta_num", "str_beta_spr", "ti_beta_bin",
               "ti_beta_num", "ti_beta_spr", "grid_beta_bin", "grid_beta_num", "grid_beta_spr",
               "depth_beta_bin","depth_beta_num", "depth_beta_spr", "gm_beta_bin", "gm_beta_num", "gm_beta_spr",
               "wm_beta_bin", "wm_beta_num", "wm_beta_spr", "subj", "hand", "age", "gender", "race", 'n_tp')
net_betas <- data.frame(matrix(ncol = length(col_names), nrow = 0))
colnames(net_betas) = col_names
col_names = c("subj", "fc_measure", "band", 'band_measure', "str_beta_bin", "str_beta_num", "str_beta_spr", "ti_beta_bin",
               "ti_beta_num", "ti_beta_spr", "grid_beta_bin", "grid_beta_num", "grid_beta_spr",
               "depth_beta_bin","depth_beta_num", "depth_beta_spr", "gm_beta_bin", "gm_beta_num", "gm_beta_spr",
               "wm_beta_bin", "wm_beta_num", "wm_beta_spr", "elec", "region", "wm", "n_tp", "elec_in_soz", "elec_spike", 
              "age", "gender", "race", "hand", "type" )
...
for (s in subj){
  ...
  if (file.exists(curr_csv) ){
    ...
    if (dim(node_data)[1] > 0){
      ...
      tmp_spike = group_by(node_data, elec_in_ict, sess_exper, time, spike_num, band_measure) %>% 
      ...
      curr_spike <- group_by(tmp_spike, sess_exper, time, band_measure) %>%
        dplyr::summarise(str_spike = mean(str_spike, na.rm=TRUE), str_not_spike = mean(str_not_spike, na.rm=TRUE))
      ...
      try({
        ...
        tmp_type <- group_by(tmp_type, sess_exper, time, band_measure) %>%
          dplyr::summarise(str_grid = mean(str_grid, na.rm=TRUE), str_depth = mean(str_depth, na.rm=TRUE))
        net_data = merge(net_data, tmp_type, 
                         by = c('sess_exper', 'time', 'band_measure'))
      })
      ...
      tmp_wm <- group_by(tmp_wm, sess_exper, time, band_measure) %>%
        dplyr::summarise(str_gm = mean(str_gm, na.rm=TRUE), str_wm = mean(str_wm, na.rm=TRUE))
      net_data = merge(net_data, tmp_wm, 
                       by = c('sess_exper', 'time', 'spike_num','spike_spread', 'bin_spike', "fc_measure", 'band', 'band_measure'))
      })
      ...
      try({
          curr_node_beta <- group_by(node_data, elec, band_measure) %>%
              ...
              dplyr::summarise(n_tp = length(time), elec_spike = mean(elec_has_spike), 
                               elec_in_soz = elec_in_soz[1], elec_in_ict = elec_in_ict[1], str_soz_beta_bin = str_soz_beta_bin[1], 
