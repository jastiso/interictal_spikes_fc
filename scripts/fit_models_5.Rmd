...
get_beta <- function(d, ys, f) {
  ...
  for (y in ys){
    ...
    if (all(is.na(d[y]))){ # check for nans
      ...
      # fit
      fit <- rlm(as.formula(paste(y, f, sep='')), data=d)
...
col_names = c("subj", "fc_measure", "band", 'band_measure', "str_soz_beta_bin", "str_not_soz_beta_bin", "str_spike_beta_bin", 
              "str_not_spike_beta_bin","str_soz_beta_num", "str_not_soz_beta_num", "str_spike_beta_num", 
              "str_not_spike_beta_num", "elec", "region", "n_tp", "elec_in_soz", "elec_has_spike", "age", "gender", "race", "hand" )
...
for (s in subj){
  ...
  # log transform
  node_data$spike_num[node_data$spike_num != 0] = log(node_data$spike_num[node_data$spike_num != 0])
  ...
  try({
      curr_node_beta <- group_by(node_data, elec, elec_in_soz, elec_has_spike, band_measure) %>%
          ...
          dplyr::summarise(n_tp = length(time), str_soz_beta_bin = str_soz_beta_bin[1], str_not_soz_beta_bin = str_not_soz_beta_bin[1], 
                           ...
                           str_soz_beta_num = str_soz_beta_num[1], str_not_soz_beta_num = str_not_soz_beta_num[1], str_spike_beta_num = str_spike_beta_num[1], 
