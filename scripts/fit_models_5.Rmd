...
for (m in fc){
  print(m)
    fit = lmer(soz_beta_num ~  SOZ*band + race + gender + hand + age + (1|subj), data=dplyr::filter(soz_data, fc_measure == m))
    print(anova(fit))
}
```
```{r}
...
for (m in fc){
  print(m)
  if (m != 'ar' && m != 'xcorr'){
    fit = with(data=dplyr::filter(net_task_data, fc_measure == m), aov(nav ~  str_beta_num*band + race + gender + hand + age + Error(subj) ))
    print(summary(fit))
  } else {
