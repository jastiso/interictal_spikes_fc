...
soz_data = read.csv('/Volumes/bassett-data/Jeni/RAM/group_analysis/net_soz_data_clean.csv')
soz_data$race = as.factor(soz_data$race)
soz_data <- mutate(soz_data, band_measure = paste(band, fc_measure, sep = '_'))
spike_data = read.csv('/Volumes/bassett-data/Jeni/RAM/group_analysis/net_spike_data_clean.csv')
...
```{r}
net_task_data = read.csv('/Volumes/bassett-data/Jeni/RAM/group_analysis/net_task_data_clean.csv')
...
for (m in fc){
  print(m)
    fit = lm(nav ~  str_beta_num*fc_measure + race + gender + hand + age, data=dplyr::filter(net_task_data, band_measure == m))
    print(anova(fit))
}
```
