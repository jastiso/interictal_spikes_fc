...
d1 = dplyr::filter(net_data, band_measure == "theta_plv")
...
```{r}
net_data = read.csv(paste('/Volumes/bassett-data/Jeni/RAM/group_analysis/win_', as.character(win), '/net_data_clean.csv',sep=''))
net_data$race = as.factor(net_data$race)
soz_data = read.csv(paste('/Volumes/bassett-data/Jeni/RAM/group_analysis/win_', as.character(win), '/net_soz_data_clean.csv',sep=''))
soz_data$race = as.factor(soz_data$race)
soz_data <- mutate(soz_data, band_measure = paste(band, fc_measure, sep = '_'))
spike_data = read.csv(paste('/Volumes/bassett-data/Jeni/RAM/group_analysis/win_', as.character(win), '/net_spike_data_clean.csv',sep=''))
...
for (m in fc){
  print(m)
    fit = lmer(soz_beta_bin ~  SOZ*fc_measure + race + gender + hand + age + (1|fc_measure), data=dplyr::filter(soz_data, band == m))
    print(anova(fit))
}

length(unique(net_data$subj))
```
...
for (m in fc){
  print(m)
    fit = lmer(soz_beta_num ~  SOZ*fc_measure + race + gender + hand + age + (1|fc_measure), data=dplyr::filter(soz_data, band == m))
    print(anova(fit))
}
```
...
for (m in fc){
  print(m)
    fit = lmer(spike_beta_num ~  spike*band + race + gender + hand + age + (1|subj), data=dplyr::filter(spike_data, fc_measure == m))
    print(anova(fit))
}
...
for (m in bfc){
  ...
  stat <- t.test(filter(curr, SOZ == 'within')$soz_beta_num, filter(curr, SOZ == 'outside')$soz_beta_num, paired=TRUE)
  print(stat)
...
soz_stats = data.frame(p = unlist(soz_ps), t = unlist(soz_ts), df = unlist(soz_df), measure = bfc, sig = p.adjust(soz_ps, method="bonferroni") < 0.05)
soz_stats
...
```{r}
net_task_data = read.csv(paste('/Volumes/bassett-data/Jeni/RAM/group_analysis/win_', as.character(win), '/net_task_data_clean.csv', sep=''))
...
for (m in fc){
  print(m)
  if (m != 'ar' && m != 'xcorr'){
    fit = lmer(data=dplyr::filter(net_task_data, fc_measure == m), TH ~  str_beta_num + race + gender + hand + age + (1|band) )
    print(anova(fit))
  } else {
