...
```
Lets test explicitly for any differences with volume conduction
...
for (m in fc){
  ...
  groups = c(rep(1,each=6),rep(2,each=1),rep(3,each=2),rep(4,each=1),5,rep(6,each=8),rep(7,each=1),8,rep(9,each=2),
             rep(10,each=6),rep(11,each=3), 12)
  if (pred == 'num'){
  ...
  for (g in vars){
    ...
    betas = c(betas, max(coeffs[curr_vars]))
  }
...
reg_betas = data.frame(band_measure = unlist(all_band_meas), beta = unlist(all_betas), pred = unlist(all_preds))
reg_betas
```
plot the betas
```{r}
