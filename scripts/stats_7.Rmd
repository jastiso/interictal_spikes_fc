...
for (m in bfc){
  ...
  bands = c(bands, curr$Frequency.Band[1])
}

# corrections based on aec or aec_orth, not both
stats_pred = data.frame(p = p.adjust(ps, method='bonferroni'), r = unlist(rs), n = unlist(ns), measure = bfc, band = unlist(bands), sig = p.adjust(ps, method="bonferroni") < 0.05)
stats_pred

```
```{r}
dplyr::summarize(group_by(stats_pred, band), meanr = mean(r), meanp = mean(p))
```
...
keep_measures = c('theta_aec_ortho', 'theta_im_coh', 'alpha_aec_ortho', 'alpha_im_coh', 'beta_aec_ortho', 'beta_im_coh', 'gamma_aec_ortho', 'gamma_im_coh', 'high gamma_aec_ortho',
                  'high gamma_im_coh', 'lfp_xcorr')
bm = unique(net_data$band_measure_clean)
supp_measures = bm[which(!bm %in% keep_measures)]

node_keep_measures = c('theta_aec_ortho', 'theta_im_coh','alpha_aec_ortho', 'alpha_im_coh', 'beta_aec_ortho', 'beta_im_coh', 'gamma_aec_ortho', 'gamma_im_coh', 'hg_aec_ortho', 
                       'hg_im_coh', 'broadband_xcorr')
...
for (m in fc){
  ...
  nObs = dim(curr)[1]
  nRow = dim(resid)[1]
  fit = lm(data=curr, paste('pca1 ~ str_beta_', pred, '+ race + gender + age + hand', sep=''))
  print(anova(fit))
...
for (m in fc){
  ...
  if (pred == 'num'){
    ...
    fit = aovp(data=curr, str_beta_spr ~ Etiology + race + gender + age)
  }
...
for (m in fc){
  print(m)
  if (pred == 'bin'){
    fit = lmer(data=dplyr::filter(sys_data, band_measure == m, cov == 't'), scale_bin ~ sys + (1|subj))
  } else if (pred == 'num') {
