...
Reliability in different etiologies
```{r}
pred = 'spr'# bin or num or spr
et = 'MCD'
aec_flag = 'aec$' # which aec do you want to EXCLUDE, orth or regular. The carrot mean "starts with", $                      means 'ends with'
...
et_data = read.csv(paste('/Volumes/bassett-data/Jeni/RAM/group_analysis/win_', as.character(win), '/clinical', detector, '.csv', sep=''))
et_data <- mutate(et_data, band_measure = tolower(paste(Frequency.Band, Measure, sep = '_')))

ts = list()
ts_all = list()


curr_measures = et_data[!grepl(aec_flag, et_data$Measure) & !grepl(coh_flag, et_data$Measure) & !grepl(plv_flag, et_data$Measure), 'band_measure']
bfc = unique(curr_measures)
for (m in fc){
  print(m)
  curr <- filter(et_data, band_measure == m, Etiology==et)
  y = unlist(na.omit(curr[paste('str_beta_', pred, sep='')]))
  stat <- oneSamplePermutationTest(y)
  ts = c(ts, stat$statistic)
  curr <- filter(et_data, band_measure == m)
  y = unlist(na.omit(curr[paste('str_beta_', pred, sep='')]))
  stat <- oneSamplePermutationTest(y)
  ts_all = c(ts_all, stat$statistic)
}

# corrections based on aec or aec_orth, not both
stats = data.frame(all = unlist(ts_all), t = unlist(ts), df = unlist(df), measure = fc)

cor.test(stats$t,stats$all,method='spearman')
...
vars = c('locus', 'hem', 'race', 'gender', 'age', 'Etiology', 'Lesional','SeizureAge',
                'contacts','Institution', 'hand','rate')
if (supp_flag){
  # loop through other measures and remove...probably a better way to do this
  for (m in keep_measures){
...
for (m in fc){
  ...
  x = model.matrix(~locus + hem + race + gender + age + Etiology + Lesional + SeizureAge + contacts + Institution + hand + rate, curr)[, -1]
...
ggplot(data=reg_betas,aes(x=band_measure, y=(beta), color=pred)) + 
  ...
  scale_color_manual(values=colorRampPalette(brewer.pal(12,'Set1'))(12)) +
