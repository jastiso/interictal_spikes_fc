%% Count the percent spike windows
clear
clc
close all
warning ON
                ...
                for n = 1:numel(sessions)
                    ...
                    if exist([data_dir, 'data_clean.mat'], 'file') && (exist([data_dir, 'spike_info', detector, '.mat'], 'file') || exist([data_dir, 'spike_info', num2str(win), '.mat'], 'file'))
                        ...
                        if ~isempty(ft_data.trial)
                            ...
                            spike_perc(cnt) = sum(spike_index)/numel(spike_index);
                            cnt = cnt + 1;
                        end
                    end
                end
...
histogram(nonzeros(spike_perc))
save([top_dir, 'n_spike_wins', detector, '.mat'], 'spike_perc');
